...
################################################################################################
def find_node_type(file_name, read_unl_dict):
    ...
    supported_node_templates = ["c7200", "mikrotik"]
    node_pair = []
    node_commands = "\n"
    ...
    for item in range(0, len(nodes)):
        ...
        # Pass variables to dynamips function
        c7200(node_id, node_name, node_type, node_template, interfaces, node_ip_config, 
                 read_unl_dict, node_pair, node_commands, supported_node_templates)
        mikrotik(node_id, node_name, node_type, node_template, interfaces, node_ip_config, 
                 read_unl_dict, node_pair, node_commands, supported_node_templates)
...
################################################################################################
def c7200(node_id, node_name, node_type, node_template, interfaces, node_ip_config,
             read_unl_dict, node_pair, node_commands, supported_node_templates):
    
    if node_template == "c7200":
        node_commands += "!\n"
        node_commands += "conf t\n"
        node_commands += " interface loopback0\n"
        node_commands += f"  ip address 10.1.1.{node_id} 255.255.255.255\n"
        node_commands += f"!\n"
        ...
        for i in range (0, len(interfaces)):
            ...
            for item in range(0, len(networks)):
                ...
                if main_net_id == interface_net_id and "pnet" not in main_net_type:
                    ...
                    for item in range(0, len(next_nodes)):
                        ...
                        next_node_template = next_node["@template"]
                        next_node_interfaces = next_node["interface"]
                        
                        # Self node id not eq to next node id #
                        if next_node_id != node_id and next_node_template in supported_node_templates:                                
                            for i in range (0, len(next_node_interfaces)):
                                ...
                                if next_node_net_id == interface_net_id:
                                    node_pair1 = node_id + next_node_id ### Example: 12
                                    node_pair2 = next_node_id + node_id  ### Example: 21
                                    if node_pair1 not in node_pair and node_pair2 not in node_pair:
                                        node_pair.append(node_pair1)
                                        node_commands += f"  ip address 10.{node_id}.{next_node_id}.1 255.255.255.252\n"
                                        node_commands += f"  description **Connected-to-{next_node_name}**\n"           
                                    elif node_pair1 in node_pair or node_pair2 in node_pair:
                                        node_commands += f"  ip address 10.{next_node_id}.{node_id}.2 255.255.255.252\n"
                                        node_commands += f"  description **Connected-to-{next_node_name}**\n"      

                    node_commands += f"  no shutdown\n"
                    node_commands += f"!\n"
                
                elif main_net_id == interface_net_id and "pnet" in main_net_type:
                    node_commands += f" interface {interface_name}\n"
...
################################################################################################
def mikrotik(node_id, node_name, node_type, node_template, interfaces, node_ip_config,
             read_unl_dict, node_pair, node_commands, supported_node_templates):
    
    if node_template == "mikrotik":
        node_commands += f"/interface bridge add name=loopback0\n"
        node_commands += "\n"
        node_commands += f"/ip address\n"
        node_commands += f"add address=10.1.1.{node_id}/32 interface=loopback0\n"
        # Start to iterate node > interface #
        for i in range (0, len(interfaces)):
            interface = interfaces[i]
            interface_name = interface["@name"]
            interface_name = interface_name.replace('eth', 'ether')
            interface_net_id = interface["@network_id"]
            # Find dict for networks #
            networks = read_unl_dict["lab"]["topology"]["networks"]["network"]
            ...
            for item in range(0, len(networks)):
                ...
                main_net_type = network["@type"]
                if main_net_id == interface_net_id and "pnet" not in main_net_type:
                    # mkt_intf_name = f"{interface_name}-TO-XXX"
                    # node_commands += f"set {interface_name} name={mkt_intf_name}\n"
                        ...
                        if next_node_id != node_id and next_node_template in supported_node_templates:                                
                            for i in range (0, len(next_node_interfaces)):
                                ...
                                if next_node_net_id == interface_net_id:
                                    ...
                                    if node_pair1 not in node_pair and node_pair2 not in node_pair:
                                        node_pair.append(node_pair1)
                                        node_commands += f"add address=10.{node_id}.{next_node_id}.1/30 interface={interface_name} comment=**Connected-to-{next_node_name}**\n"           
                                    elif node_pair1 in node_pair or node_pair2 in node_pair:
                                        node_commands += f"add address=10.{next_node_id}.{node_id}.2/30 interface={interface_name} comment=**Connected-to-{next_node_name}**\n"
                
                elif main_net_id == interface_net_id and "pnet" in main_net_type:
                    node_commands += f"add address=10.99.99.{node_id}/16 interface={interface_name} comment=**Connected-to-MGMT**\n"
                    node_commands += f"\n"
                # else:
                #     print("Error: Check condition!")
...
def eve_login():
    ...
    find_node_type(file_name, read_unl_dict)
