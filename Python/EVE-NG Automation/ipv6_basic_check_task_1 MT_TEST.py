...
def task1_check(hostname, IP):
    try:
        ...
        with ConnectHandler(**cisco_devices) as ssh:
            ...
            with open(hostname + "-task1-compare.txt", "w") as saveoutput:
                saveoutput.write(output1)
                saveoutput.write("\n")
                saveoutput.write(output2)
                saveoutput.write("\n")

            with open(hostname + "-task1-compare.txt", "r") as readoutput:
            ...
            if IntStatusStart in text and IntStatusEnd in text:
                ...
                print("Couldn't read Interface Status of " + hostname)
            ...
            if GUAStart in text and GUAEnd in text:
                ...
                if GUA in GUAAddress and GUACount == 1 and PrefixLenValue == 64:
                    savereport.write(hostname + " IPv6 address and prefix length are correct.\n")

                elif GUA in GUAAddress and GUACount == 1 and PrefixLenValue != 64:
                    savereport.write(hostname + " IPv6 address is correct but prefix length is not correct.\n")

                elif GUA in GUAAddress and GUACount > 1:
                    savereport.write(hostname + " has multiple IPv6 configured.\n")

                else:
                    savereport.write(hostname + " IPv6 address is not correct.\n")

                if IntStatus == "up/up":
                    savereport.write(hostname + " interface Fa0/0 status is UP.\n")
                else:
                    savereport.write(hostname + " interface Fa0/0 status is DOWN.\n")

            else:
                savereport.write("\n")
                savereport.write(hostname + " IPv6 address is not configured.")
                savereport.write("\n")

    except Exception as e:
        savereport.write("\n")
        savereport.write(hostname + f" Error: {str(e)}")
        savereport.write("\n")


def main():
    for hostname, IP in zip(HostnameList, IPList):
        task1_check(hostname, IP)
